...
class AttentionBlock(Layer):
    ...
    def call(self, x, training, mask=None):
        ...
        attn_output, _ = self.attention_layer(x,x,x,mask)
        ...
        ff_output = self.activation_layer(self.cnn_layer(x))
        if not self.last:
            ff_output = self.dense(ff_output)
            ff_output = self.dropout2(ff_output, training=training)
            x = self.layernorm2(x + ff_output)
        else:
            x = self.layernorm2(ff_output)
